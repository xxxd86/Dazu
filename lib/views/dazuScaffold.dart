...
//页面控制器
class ScaffoldController extends ValueNotifier<PagePositon> {
  ScaffoldController([
    PagePositon value = PagePositon.middle,
  ]) : super(value);

  Future? animateToPage(PagePositon pagePositon) {
    return _onAnimateToPage?.call(pagePositon);
  }

  Future? animateToLeft() {
    return _onAnimateToPage?.call(PagePositon.left);
  }

  Future? animateToRight() {
    return _onAnimateToPage?.call(PagePositon.right);
  }

  Future? animateToMiddle() {
    return _onAnimateToPage?.call(PagePositon.middle);
  }

  Future Function(PagePositon pagePositon)? _onAnimateToPage;
}
class dazuScaffold extends StatefulWidget {
  ...
  const dazuScaffold({super.key, this.controller, this.header, this.tabBar, this.leftPage, this.rightPage,  this.currentIndex = 0,  this.hasBottomPadding = false, this.enableGesture, this.page, this.onPullDownRefresh});

  @override
  State<dazuScaffold> createState() => _dazuScaffoldState();
}

class _dazuScaffoldState extends State<dazuScaffold> with TickerProviderStateMixin{
  ...
  Future animateToPage(p) async {
    ...
    switch (p) {
      case PagePositon.left:
        await animateTo(screenWidth!);
        break;
      case PagePositon.middle:
        await animateTo();
        break;
      case PagePositon.right:
        await animateTo(-screenWidth!);
        break;
    }
  ...
  Widget build(BuildContext context) {
    ...
    body = WillPopScope(
      ...
      child: Scaffold(
        body: body,
        backgroundColor: Color(0x7770ccc3),
  ...
  onHorizontalDragEnd(details, screenWidth) {
    ...
    if (vOffset > scrollSpeed && inMiddle == 0) {
      // 去右边页面
      return animateToPage(PagePositon.left);
    } else if (vOffset < -scrollSpeed && inMiddle == 0) {
      // 去左边页面
       return animateToPage(PagePositon.right);
    } else if (inMiddle > 0 && vOffset < -scrollSpeed) {
      return animateToPage(PagePositon.middle);
    } else if (inMiddle < 0 && vOffset > scrollSpeed) {
      return animateToPage(PagePositon.middle);
    }
...
class _LeftPageTransform extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Transform.scale(
      scale: 0.88 + 0.1 * offsetX! / screenWidth < 0.5
          ? 0.88
...
class _MiddlePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
      ...
      _headerContain = Opacity(
        ...
        child: Transform.translate(
          ...
          child: SafeArea(
            child: Container(
              height: 44,
              child: header ?? Placeholder(color: Colors.blue),
            ),
          ),
        ),
      );
    ...
    Widget middle = Transform.translate(
      //offsetX! / 5
      offset: Offset(offsetX! > 0 ? offsetX! : 0, 0),
      child: Stack(
        children: <Widget>[
          Container(
            child: Stack(
